...
--Firstly, I'll create Employees table and then insert into records.
...
--to find top 5 employees from the Employees table
...
--Firstly, I'll create Customers table and then insert values into the table
...
--6.Write a query that uses the WHERE clause to filter for employees with Salary >= 60000 
--and Department = 'HR'.
...
--17.Write a query that filters the Products table for ProductName values 
--containing the letter 'e' using LIKE.
...
select * from Customers order by City, PostalCode desc;


--Hard-Level Tasks(8)
--20.Write a query that selects the top 10 products with the highest sales, 
--using TOP(10) and ordered by SalesAmount DESC.
--Firstly, I'll create Sales table and then insert valus into the table.
...
/*
The fields and values of the Sales table were given by instructors and in the table, the column name
isn't SalesAmount, it is SaleAmount
*/
select top 10 * from Sales order by SaleAmount desc;

--21.Combine FirstName and LastName into one column named FullName in the Employees table. 
...
--23.Write a query that selects products whose Price is less than 10% of the average price 
--in the Products table. (Do some research on how to find average price of all products)
select * from Products where Price < (select avg(Price)*0.1 from Products);

--24.Use WHERE clause to filter for employees whose Age is less than 30 and who work 
--in either the 'HR' or 'IT' department.
...
--26.Write a query that uses the ALL operator to find employees whose salary is greater 
--than all employees in the 'Sales' department.
/*
in Employees table, it isn't 'Sales' derpartment, but the table has a 'Marketing' department
and I'll use it instead of 'Sales'
*/
select * from Employees 
where Salary > all(select Salary from Employees where DepartmentName = 'Marketing');


--27.Write a query that filters the Orders table for orders placed in the last 180 days 
--using BETWEEN and CURRENT_DATE. (Search how to get the current date)
--Firstly, I'll create Orders table and then insert values into the table
...
--in sql server, getdate is used instead of current_date
